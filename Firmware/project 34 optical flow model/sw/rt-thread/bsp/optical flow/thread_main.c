...
void entry_thread_can_rx(void* parameter)
{
	...
	extern void altitude_angle_decode(CanRxMsg rx);
	...
	while(1)
	{
		...
		switch(RxMessage.StdId & CAN_FILTER)
		{
			...
			case CAN_ALT_ANGLE: 
				altitude_angle_decode(RxMessage);
...
void entry_thread_can_tx(void* parameter)
{
	...
	while(1)
	{
		...
		TxMessage.StdId = CAN_TYPE_SLAVER | CAN_OPT | CAN_MESSAGE_INDEX0;
		TxMessage.ExtId = 0x00;
		TxMessage.RTR = CAN_RTR_DATA;
		TxMessage.IDE = CAN_ID_STD;
		TxMessage.DLC = 8;		
		
		memcpy(&TxMessage.Data[0], &sum_x, 4);
		memcpy(&TxMessage.Data[4], &sum_y, 4);
