...
ALIGN(RT_ALIGN_SIZE)
static rt_uint8_t as_stack[4096];
static struct rt_thread as_thread;
static void as_thread_entry(void* parameter)
{
	thread_as();
}
...
{
    /* init main thread */
	rt_thread_init(&as_thread,
		"air speed",
		as_thread_entry, 
		RT_NULL,
		(rt_uint8_t*)&as_stack[0], 
		sizeof(as_stack), 6, 1);
    rt_thread_startup(&as_thread);
}
